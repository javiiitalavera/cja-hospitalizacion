...
export default function Pacientes() {
  ...
  async function fetchIngresos() {
    ...
    let query = supabase
      ...
      .select(`
        *,
        paciente:pacientes(*),
        medico_responsable:profesionales(*)
      `)
  ...
  const filtrados = ingresos.filter(i => {
    ...
    const nombre = `${i.paciente?.nombre} ${i.paciente?.primer_apellido} ${i.paciente?.segundo_apellido ?? ''}`.toLowerCase()
    return nombre.includes(busqueda.toLowerCase()) ||
      i.habitacion?.toString().includes(busqueda)
  })
    ...
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-5">
        ...
        {(['activo', 'alta', 'todos'] as const).map(e => (
          ...
          >
            {e === 'activo' ? 'Ingresados' : e === 'alta' ? 'Altas' : 'Todos'}
          </button>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-400">No hay pacientes</td></tr>
            ) : filtrados.map(i => (
              <tr key={i.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <p className="font-medium text-slate-800">
                    {i.paciente?.primer_apellido} {i.paciente?.segundo_apellido}, {i.paciente?.nombre}
                  </p>
                  <p className="text-xs text-slate-400">{i.paciente?.cipna ?? 'Sin CIPNA'}</p>
                </td>
                <td className="px-4 py-3 text-slate-600">
                  {i.medico_responsable?.nombre ?? '—'}
                </td>
                <td className="px-4 py-3 text-slate-500">
                  {i.fecha_ingreso
                    ? new Date(i.fecha_ingreso).toLocaleDateString('es-ES')
                    : '—'}
                </td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_COLOR[i.estado]}`}>
