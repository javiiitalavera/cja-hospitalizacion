...
export default function DetalleIngreso() {
  ...
  async function darAlta() {
    ...
    setIngreso(prev => prev
      ? { ...prev, estado: altaForm.estado as any, fecha_alta: altaForm.fecha_alta }
      : prev)
    setModalAlta(false)
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-500 flex-wrap">
                ...
                {ingreso.medico_responsable && (
                  <span>· {ingreso.medico_responsable.nombre} {ingreso.medico_responsable.apellidos}</span>
                )}
                <span>· {new Date(ingreso.fecha_ingreso).toLocaleDateString('es-ES')}</span>
                <span className={`px-2 py-0.5 rounded-full font-medium ${ESTADO_COLOR[ingreso.estado] ?? 'bg-slate-100'}`}>
